...
def test_mysql_schema_uses_mysql_types_and_declares_all_tables():
    ...
    for table in ("price_observations", "data_sources", "collection_logs", "schema_migrations"):
...
def test_mysql_migrations_are_tracked_and_idempotent():
    ...
    assert first_run == ["001", "002"]
    assert second_run == []
    assert connection.versions == {"001", "002"}
...
def test_mysql_backend_applies_schema_and_migrations(monkeypatch):
    ...
    assert {"price_observations", "data_sources", "collection_logs", "schema_migrations"} <= tables
